...
Coordinates the Gemini-backed PDF import workflow. It stores Part 1-7 PDF/page selections, editable prompts, answer-sheet image paths, reviewed PaddleOCR text in `import_agent_tasks.ocr`, persists each request as an SQLite import-agent task, and runs the agent call on `ImportQuestionsAgentWorker(QThread)`.
...
Each send creates one `ImportAgentTask` row per selected TOEIC part. The ViewModel starts only one worker at a time, emits a per-request success payload as soon as that request finishes, merges successful results for dialog state, then starts the next queued request. A worker error marks only the current row `failed`; later queued requests continue running and can still be saved. Failed rows remain visible in the request status dialog and checked rows can be queued together for manual retry. A non-running request can also run PaddleOCR against its saved selected question/transcript PDFs, excluding answer-sheet images. The reviewed OCR text is stored in the task row's `ocr` column, the review dialog previews the prompt built from the selected part's `PART_PROMPTS` contract, and the worker appends the reviewed OCR text to the Gemini prompt. OCR-backed retries upload only the answer-sheet image to the agent API so the saved OCR text is the complete question/transcript source.
