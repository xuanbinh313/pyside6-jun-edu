...
`ImportQuestionsAgentDialog` adds a visual PDF/page workflow for TOEIC Parts 1-7. Answer-sheet drop areas sit above the part tabs: Parts 1-4 require the listening answer sheet, and Parts 5-7 require the reading/writing answer sheet before sending. Parts 1 and 3-7 can select question PDF pages; Part 2 uses a context text input defaulting to "Mark your answer on your answer sheet" instead of question-page selection. The PDF page selector can assign checked pages to any TOEIC part from a dropdown, defaulting to the part whose button opened the dialog, and saves those pages back to the question or transcript lane based on the clicked button. Part 1 question pages are split into two image crops per selected page before upload and saved as `IMAGE_DIAGRAM` contexts. Parts 5-7 use the reading prompt contract from `ImportQuestionsViewModel.READING_PROMPT_TEXT`. The dialog includes a Requests button that opens an agent-request status table with queued/running/succeeded/failed state, attempts, errors, manual retry, refresh, and removal for non-running requests. The agent dialog delegates Gemini calls, PDF slicing, image splitting, answer-sheet attachment, SQLite task tracking, retry, and response parsing to `ImportQuestionsAgentViewModel`, then returns the same import payload contract used by the manual dialog.
